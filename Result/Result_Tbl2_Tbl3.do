...
*** Table 1 ***
file open tbl using "${output}Tbl1_Baseline_`file_name'.tex", write replace //_`time'
file write tbl "\begin{table}[h!]\centering" _n "\caption{OLS Regression Estimates for Pension Eligibility}" _n "\label{tbl-baseline}" _n "\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" _n "\resizebox{\linewidth}{!}{%" _n
...
esttab est_full est_svy3_own_health_poor0 est_svy3_own_health_poor1 est_svy3_enough_money1 est_svy3_enough_money0  using "${output}Tbl1_Baseline_`file_name'.tex", append ///_`time'
	lab b(%9.3f) se(%9.3f) star(* 0.10 ** 0.05 *** 0.01) keep(`cov') order(`cov') noomit  stat(ymean N, labels("Mean of Dep Var" "N")) nobase nogap nomtitles nonumbers fragment

file open tbl using "${output}Tbl1_Baseline_`file_name'.tex", write append //_`time'	
...
*** Table 2: LATE Stats by variables ***
...
file open tbl using "${output}Tbl2_Compliance_`file_name'.tex", write replace //_`time'
file write tbl "\begin{table}[h!]\centering" _n "\caption{Responsiveness to Pension Eligibility}" _n "\label{tbl-compliers}" _n "\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" _n "%\resizebox{\columnwidth}{!}{%" _n
file write tbl "\begin{tabular}{l*{5}{c}}" _n
file write tbl "\toprule"  _n
file write tbl " &Full   & Good   &  Poor    & Saved   &  Not    \\ " _n
file write tbl " &Sample & Health &  Health  & Enough  & Enough  \\ " _n
...
*** Row 1: P(D=1)
file write tbl  "\$P(D=1)$ & 0" (pd_full) 
foreach v in svy3_own_health_poor  svy3_notenough_money {
forvalues n = 0/1 {
file write tbl "& 0" (pd_`v'`n') 
...
*** Row 2: P(Z=1)
file write tbl  "\$P(Z=1)$ & 0" (pz_full) 
foreach v in svy3_own_health_poor  svy3_notenough_money {
forvalues n = 0/1 {
file write tbl "& 0" (pz_`v'`n') 
}
}
file write tbl  " \\ " _n "[0.5 em] " _n  "\multicolumn{6}{l}{\textbf{Estimated Percentage}}\\" _n


*** Row 3: AT
file write tbl  "Always Taker & 0" (P_AT_full) 
foreach v in svy3_own_health_poor  svy3_notenough_money {
forvalues n = 0/1 {
file write tbl "& 0" (P_AT_`v'`n') 
}
}
file write tbl  " \\ " _n "\$P(D_1=D_0=1)$ & \multicolumn{5}{l}{}\\" _n "[0.3em] " _n

*** Row 4: AT
file write tbl  "Complier & 0" (P_Com_full) 
foreach v in svy3_own_health_poor  svy3_notenough_money {
forvalues n = 0/1 {
file write tbl "& 0" (P_Com_`v'`n') 
}
}
file write tbl  " \\ " _n "\$P(D_1>D_0)$ & \multicolumn{5}{l}{}\\" _n "[0.3em] " _n

*** Row 5: NT
file write tbl  "Never Taker & 0" (P_NT_full) 
foreach v in svy3_own_health_poor  svy3_notenough_money {
forvalues n = 0/1 {
file write tbl "& 0" (P_NT_`v'`n') 
}
}
file write tbl  " \\ " _n "\$P(D_1=D_0=0)$ & \multicolumn{5}{l}{}\\" _n "[0.3em] " _n "\bottomrule" _n "\end{tabular}\\" _n "%}" _n
file write tbl  "{\footnotesize Notes: Regression estimates are from a linear probability model that includes indicators for own and spouse's health status (where appropriate), gender, marital status, age dummies, years of service`control_footnote', salary, race/ethnicity indicators, number of children, education, and agency type indicators.  See Appendix \ref{app-compliers} for a detailed explanation of how LATE parameters are calculated.}" _n
file write tbl "\end{table}" _n
file close _all
exit
*** Table 3: K-Weighted Means by LATE Groups ***
file open tbl2 using "${output}Tbl3_KWeightedMean_`Fuzzy'_bootstrap_`bootstrap_time'`restrict_to_no_early_elg'_`time'.tex", write replace //
file write tbl2 "\begin{table}[h!]\centering" _n "\caption{Mean Characteristics by LATE Groups}" _n "\label{tbl-kweight}" _n "\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" _n "\resizebox{\columnwidth}{!}{%" _n
file write tbl2 "\begin{tabular}{l*{7}{c}}" _n
file write tbl2 "\toprule"  _n
file write tbl2 " &   &              & Always       &  Never       & Diff     & Diff      & Diff  \\ " _n
file write tbl2 " &All& Complier     & Taker        &  Taker       & AT \& NT & AT \& Com & NT \& Com \\ " _n
file write tbl2 " &   & \$P(D_0<D_1)$   & \$P(D_0=D_1=1)$ & \$P(D_0=D_1=0)$ & (3)-(4)  & (3)-(2)  & (4)-(2)   \\ " _n
file write tbl2 " & (1) & (2) & (3) & (4) & (5) & (6) & (7) \\ " _n

file write tbl2 "\midrule " _n 
...
file write tbl2 " `:label _`v' 1' & 0" (round(`v'_b_mean_all,0.001)) "& 0" (round(`v'_b_mean_C,0.001)) " & 0" (round(`v'_b_mean_AT,0.001)) "  & 0"  (round(`v'_b_mean_NT,0.001)) "  & 0" (round(`v'_b_di_A_N,0.001)) "``v'_p_di_A_N'  & 0" (round(`v'_b_di_A_C,0.001)) "``v'_p_di_A_C' & 0" (round(`v'_b_di_N_C,0.001))  "``v'_p_di_N_C' \\ " _n
file write tbl2 "  & (0" (round(`v'_se_mean_all,0.001)) ") &(0" (round(`v'_se_mean_C,0.001)) ") &(0" (round(`v'_se_mean_AT,0.001)) ") &(0"  (round(`v'_se_mean_NT,0.001)) ") &(0" (round(`v'_se_di_A_N,0.001)) ") &(0" (round(`v'_se_di_A_C,0.001)) ") &(0" (round(`v'_se_di_N_C,0.001))  ") \\ " _n

}
file write tbl2 "\midrule " _n 
file write tbl2 " Estimated Portion  & 1 & 0" (round(b_P_COM,0.001))  " & 0" (round(b_P_AT,0.001)) " & 0" (round(b_P_NT,0.001)) " & & &  \\ " _n
...
file write tbl2 "\end{tabular}" _n "}"_n
file write tbl2 "{\footnotesize Notes: Standard errors from a nonparametric bootstrap with `bootstrap_time' replications are in parentheses.}" _n
file write tbl2 "\end{table}"_n  
file close _all


*** Summery Stats by subgroup
...
local cov "D Z `Z_control' svy3_own_health_poor svy3_sphlthpoor admin3_female svy3_num_kids_12 svy3_num_kids_34 svy3_race_black svy3_race_hisp svy3_race_other  svy3_educ_ba admin3_atype_school admin3_atype_stategovt admin3_total_salary_10K_2015  admin3_yos_at_svy3 age2 age3 age4 age5 age6 age7 age8 age9 age10 age11 age12 age13"
